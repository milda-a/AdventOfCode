...
def evaluate(wire, instructions):
    if wire_signals.get(wire, None):
        ...
        wire_signals[wire] = evaluate(wire_signals[instructions[0]], wire_instructions[instructions[0]]) if \
        instructions[0].isalpha() else int(instructions[0])
        return wire_signals[wire]
    else:
        if "NOT" in instructions:
            wire_signals[wire] = ~evaluate(instructions[1], wire_instructions[instructions[1]])
        else:
            i_1 = evaluate(instructions[0], wire_instructions[instructions[0]]) if instructions[0].isalpha() else int(
                instructions[0])
            i_2 = evaluate(instructions[2], wire_instructions[instructions[2]]) if instructions[2].isalpha() else int(
                instructions[2])
