...
def part_1():
    ...
    symbols = [[x for x in each_set if not x.isdigit() and x != "."] for each_set in input_unique]
    all_symbols = []
    for i in symbols:
        all_symbols += i
            ...
            if each_char.isdigit():
                ...
                if seeking_digits:
                    for k in range(max(y - 1, 0), min(y + 2, len(input_amended))):
                        for l in range(max(x - len(seeking_digits) - 1, 0), min(x + 1, len(line))):
...
def part_2():
    ...
    input_amended = [x + "." for x in input]  # to not deal with end of line numbers when iteration runs out
    for y, line in enumerate(input_amended):
        seeking_digits = ""
        for x, each_char in enumerate(line):
            if each_char.isdigit():
