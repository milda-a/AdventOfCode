    ...
    if reverse:
        for x in range(len(original_array)):
            for y in range(len(original_array) - 1, -1, -1):
                diag_data[x - y + len(original_array) - 1].append(
                    str(original_array[x][y])
                )
...
def check_chunky(chunk):
    ...
    if (
        find_all_occurrences("(MAS)", diag) > 0
        or find_all_occurrences("(SAM)", diag) > 0
    ):
        if (
            find_all_occurrences("(MAS)", rev) > 0
            or find_all_occurrences("(SAM)", rev) > 0
        ):
            return True
    return False
...
if __name__ == "__main__":
    ...
    # up and down
    up_and_down_data = [
        "".join([data[i][j] for i in range(len(data))]) for j in range(len(data))
    ]
